...
def sample_linspace_frames_from_video(video_path: str, num_frames: int = 8, size: int = 224) -> torch.Tensor:
    ...
    # Fill missing frames using last_good / first available
...
class UNBAVQ2013Exp3Dataset(Dataset):
    def __init__(self,
                 video_dir,
                 mos_csv_path,
        ...
        """
        Args:
            video_dir (str): Directory containing .avi video files (UnB-AVQ-2013 Exp3)
            mos_csv_path (str): Path to UnB-AVQ-2013-Experiment3.csv (columns: Source,Video,Audio,MOS)
            scoreq_reliability_csv_path (str): CSV with columns: filename,scoreq_reliability
        ...
        # ========== Load MOS (Mean Opinion Score) ==========
        # For 2013 CSV, rows identify a condition by (Source, Video, Audio).
        # We match using the video filename structure: avX_*_vbY_abZ.avi -> (Source=avX, Video=vbY, Audio=abZ)
        self.quality_map = {}
        delim = ';'
        try:
            with open(mos_csv_path, 'r', newline='', encoding='utf-8') as f:
                sniff = csv.Sniffer().sniff(f.read(2048), delimiters=[',', ';', '\t'])
        ...
        with open(mos_csv_path, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                src = (row.get('Source') or row.get('source') or '').strip().lower()
                vid = (row.get('Video') or row.get('video') or '').strip().lower()
                aud = (row.get('Audio') or row.get('audio') or '').strip().lower()
                mos_str = (row.get('MOS') or row.get('mos') or '').strip()

                if not (src and vid and aud and mos_str):
                    continue

                try:
                    mos = float(mos_str)
                    if np.isfinite(mos):
                        self.quality_map[(src, vid, aud)] = mos
                except Exception:
                    continue

        print(f"✅ Loaded MOS for {len(self.quality_map)} samples from {mos_csv_path}")

        # ========== Load ScoreQ Audio Reliability (filename -> reliability) ==========
        self.reliability_map = {}
        with open(scoreq_reliability_csv_path, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                ...
                if not filename:
                    continue
                key = os.path.basename(filename).replace('.avi', '').strip()

                try:
                    reliability = float(row.get('scoreq_reliability', 0.5))
                    if np.isfinite(reliability):
                        # ✅ enforce minimum 0.1 (your requirement)
                        reliability = float(np.clip(reliability, 0.1, 1.0))
                        self.reliability_map[key] = reliability
                    else:
                        self.reliability_map[key] = 0.5
                except Exception:
                    self.reliability_map[key] = 0.5
        ...
        # ========== Video augmentation pipeline (same as datasets.py) ==========
        ...
        # ========== Audio augmentation (same as datasets.py) ==========
    ...
    def __getitem__(self, idx):
        ...
        # Parse (Source, Video, Audio) from video_name
        parts = video_name.lower().split('_')
        src = parts[0]
        vid = parts[-2]
        aud = parts[-1]

        # Get MOS and reliability
        mos = self.quality_map.get((src, vid, aud), 50.0)

        # Normalize MOS to [0,1] (2013 MOS looks like 0..100)
        if not np.isfinite(mos):
            print(f"❌ DATASET: Non-finite MOS for {video_name}: {mos}")
            mos = 50.0
        normalized_quality = float(np.clip(mos / 100.0, 0.0, 1.0))
        ...
        audio_reliability = float(np.clip(audio_reliability, 0.1, 1.0))  # enforce >= 0.1

        # ========== Decode 8 frames from AVI (NO .pt frames needed) ==========
        try:
        ...
        # Apply video augmentation if training (same style)
        if self.train:
            aug_frames = []
